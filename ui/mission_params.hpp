class Params
{
	...
	class ace_medical_AIDamageThreshold { //This needs to match an ace_setting, this one is a "SCALAR"(number)
        title = "Ain't no rest to the wicked (20% more hitpoints for AI)"; // Name that is shown
        ACE_setting = 1; //Marks param to be read as an ace setting, without this nothing will happen!
        values[] = {1, 1.2}; //Values that ace_medical_level can be set to
        texts[] =  {"No", "Yes"}; //Text names to show for values (Basic will set level to 1, Advanced will set level to 2)
